 ...
 */
   ...
   function changeAreaList(idx, obj){	
	...
	function normalSearch2(currentPage){
		...
		url = "/common/juso/AjaxRequestXML.jsp?getUrl="+escape("http://www.juso.go.kr/link/search.do?extend=false&mode=jibun_search&searchType=location_jibun&topTab=1&engineCtpNm="+encodeURI(form.engineCtpNm.value)+"&engineSigNm="+encodeURI(form.engineSigNm.value)+"&engineEmdNm="+encodeURI(form.engineEmdNm.value)+"&engineLiNm="+encodeURI(form.engineLiNm.value)+"&engineBdMaSn="+form.engineBdMaSn.value+"&engineBdSbSn="+form.engineBdSbSn.value+"&engineMtYn="+form.engineMtYn.value+"&currentPage="+currentPage);
	...
	*/
	 ...
	 function setOpenerValue(newaddr){
		 ...
		 <%
			...
			if( "placard1".equals(gubun) ) {
				post1 = "zipcode1";
				post2 = "zipcode2";
				add1 = "address1";
				add2 = "address2";
			%>
				window.opener.document.getElementById("zipcode").value = zipcode[0]+zipcode[1];
			<%
			} else if( "placard2".equals(gubun) ) {
				post1 = "zip1";
				post2 = "zip2";
				add1 = "com_addr1";
				add2 = "com_addr2";
			%>
				window.opener.document.getElementById("com_zipcode").value = zipcode[0]+zipcode[1];
			<%
			} else if( "board".equals(gubun) ) {
				post1 = "zipcode1";
				post2 = "zipcode2";
				add1 = "boardData_address1";
				add2 = "boardData_address2";
			%>
				window.opener.document.getElementById("boardData_zipcode").value = zipcode[0]+zipcode[1];
			<%
			}
		...
		%>
		 ...
		 window.opener.document.getElementById("<%= add1 %>").value = result[0];
		 window.opener.document.getElementById("<%= add2 %>").value = document.getElementById("detail").value;
	 ...
	 function selectSearch(mode) {
		 if (mode == 'road'){
				document.location.href = "./road.jsp?gubun=<%= gubun %>";
			}else if(mode =='jibun'){
				document.location.href = "./jibun.jsp?gubun=<%= gubun %>";
			}else if(mode=='sangho'){
				document.location.href = "./sangho.jsp?gubun=<%= gubun %>";
			}		
	}
	...
	<div id="container">
		<div id="content">
			<ul class="tab">
				...
				<form name="check" method="post" onsubmit="return normalSearch('1')">
					...
					<fieldset>
						<legend>정보 입력</legend>
						<div class="group">
						<p style="color:#0861bc; text-align:left; margin:0 0 10px 50px;">※ 선택하신후에 <strong style="color:#bb0000;">선택 버튼</strong>을 눌러주세요</p>
						<p class="input">
							<label for="city1"><strong>시&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;도</strong></label>
							<select title="시/도 선택"  name="city1" id="city1" style="width:185px" onclick="javascript:changeAreaList(1, document.getElementById('city1'));">
							...
							<a href="#city1" onclick="javascript:changeAreaList(1, document.getElementById('city1'));"><img src="/common/images/category_submit.gif" alt="선택"/></a>
						</p>
						<p class="input">
							<label for="county1"><strong>시&nbsp;&nbsp;군&nbsp;&nbsp;구</strong></label>
							<select title="시/군/구 선택"  name="county1" id="county1" style="width:185px" onclick="changeAreaList(1, document.getElementById('county1'));">
							<option value="">::선택::</option>
							<c:forEach items="${county}" var="county">
								<option value="${county.countyId}">${county.countyName}</option>
							</c:forEach>
							</select>
							<a href="#county1" onclick="changeAreaList(1, document.getElementById('county1'));"><img src="/common/images/category_submit.gif" alt="선택"/></a>
						</p>
						<p class="input">
							<label for="town1_oldaddr"><strong>읍&nbsp;&nbsp;면&nbsp;&nbsp;동</strong></label>
							<select id="town1_oldaddr" name="town1_oldaddr"  title="읍면동 선택" style="width:185px" onclick="changeAreaList(1, document.getElementById('town1_oldaddr'))"> 
								<option value="">::선택::</option>
											<c:forEach items="${town}" var="county">
												<option value="${town.townId}" >${town.townName }</option>
											</c:forEach>
							</select>
							<a href="#town1_oldaddr" onclick="changeAreaList(1, document.getElementById('town1_oldaddr'))"><img src="/common/images/category_submit.gif" alt="선택"/></a>
						</p>
						<p class="input">
							<label for="ri1_oldaddr"><strong>리</strong></label>
							<select id="ri1_oldaddr" name="ri1_oldaddr" title="리 선택"  class="small" style="width:130px">  
								<option value="">::선택::</option>
							</select>
						...
						<p class="input">
							...
							<input type="text" name="bun1" value="" id="bun1" class="text" style="width:84px"  title="번지 앞자리 입력"/> - <input type="text" name="bun2" value="" id="bun2" class="text" title="번지 뒷자리 입력" style="width:84px"/>
							
						 </p>
						</div>
						<div class="btn">
							<input type="image" src="http://www.juso.go.kr/images/link/btn_search.gif" alt="검색"/>
						</div>
						<div >
							<img src="http://www.juso.go.kr/images/test.gif" width="380" height="85" alt="올바른 도로명주소표기법 / 공동주택(아파트등) 서울시 서초구 반포대로 58, 101동 501호(서초동, 서초아트자이) / 주택,상가 서울특별시 서초구 반포대로23길6(서초동)" />
							자세한 표기법은   <a href="http://www.juso.go.kr" style="font-size: 10px" target="_blank" title="새창">도로명주소안내홈페이지</a> 를 참조하세요
						</div>
					</fieldset>
					</div>
				</form>
